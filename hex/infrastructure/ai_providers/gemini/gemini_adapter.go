...
var defaultSystemPromps string = "You are a helpful assistant, help the user with their enquiries"

type geminiConfig struct { //  Config struct for adapter initialization
	APIKey            string         `validate:"required"`
	ModelName         string         `validate:"required"`
	SystemInstruction *genai.Content `validate:"required"`
...
func NewGeminiConfig(apiKey string, modelName string, systemInstruction *string, maxOutputTokens int32) (*geminiConfig, error) {
	configStruct := geminiConfig{}

	configStruct.candidateNumber = 1

	if systemInstruction == nil {
		systemInstruction = &defaultSystemPromps
	}

	configStruct.SystemInstruction = &genai.Content{
...
func (ad *GeminiAdapter) serializeResponse(res *genai.GenerateContentResponse) (*inputport.AIChatResponse, *outputport.AIResposeMetadata[any], error) {
	...
	msg, err := entities.NewMessage(entities.AIBotChatMessageCode, finalMessage)
